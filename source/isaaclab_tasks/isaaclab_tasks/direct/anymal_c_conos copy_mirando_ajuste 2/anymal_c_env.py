...
class AnymalCFlatEnvCfg(DirectRLEnvCfg):
    ...
    foot_force_reward_scale = -0.1  # Penalize foot force with a scale of -0.1
...
class AnymalCEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._rear_rigth_foot_id, _ = self._contact_sensor.find_bodies(".*RR_foot")
        self._front_rigth_foot_id, _ = self._contact_sensor.find_bodies(".*FR_foot")
        self._front_left_foot_id, _ = self._contact_sensor.find_bodies(".*FL_foot")
        self._rear_left_foot_id, _ = self._contact_sensor.find_bodies(".*RL_foot")
        ...
        self.goal_reached_bonus = 6.0  # 10.0
        self.position_progress_weight = 10 #5.0  # 1.0  # 5000.0
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        constant_penalty = torch.full((self.num_envs,), -1 * self.step_dt, device=self.device)
        ...
        front_right_foot_contact = torch.sum(torch.square(torch.max(
            torch.norm(net_contact_forces[:, :, self._front_rigth_foot_id], dim=-1), dim=1  # type: ignore
        )[0]),dim=1)
        
        front_left_foot_contact = torch.sum(torch.square(torch.max(
            torch.norm(net_contact_forces[:, :, self._front_left_foot_id], dim=-1), dim=1  # type: ignore
        )[0]),dim=1)

        back_right_foot_contact = torch.sum(torch.square(torch.max(
            torch.norm(net_contact_forces[:, :, self._rear_rigth_foot_id], dim=-1), dim=1  # type: ignore
        )[0]),dim=1)
        
        back_left_foot_contact = torch.sum(torch.square(torch.max(
            torch.norm(net_contact_forces[:, :, self._rear_left_foot_id], dim=-1), dim=1  # type: ignore
        )[0]),dim=1)
        ...
        rewards = {
            ...
            * self.step_dt,
            ...
            "front_right_foot_contact": front_right_foot_contact * self.cfg.foot_force_reward_scale * self.step_dt,
            "front_left_foot_contact": front_left_foot_contact * self.cfg.foot_force_reward_scale * self.step_dt,
            "back_right_foot_contact": back_right_foot_contact * self.cfg.foot_force_reward_scale * self.step_dt,
            "back_left_foot_contact": back_left_foot_contact * self.cfg.foot_force_reward_scale * self.step_dt,
        }
